...
class TestJSONEncoder : XCTestCase {
    ...
    func test_numericLimits() {
        ...
        @discardableResult
        func decode(_ type: String, _ value: String) throws -> DataStruct {
            var key = type.lowercased()
            key.append("Value")
            return try JSONDecoder().decode(DataStruct.self, from: "{ \"\(key)\": \(value) }".data(using: .utf8)!)
        }
        ...
        let badValues = [
            ...
            ("UInt64", "-1"), ("UInt64", "18446744073709551616"),
        ]
